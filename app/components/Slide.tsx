...
const generateSelectItemV2 = (images:string[], selectedItem: number, handdleSelectedItem: (number:number) => void) => {
  return (
    images.map( (img, index) =>
    <a
      onClick={() => {handdleSelectedItem(index)}}
...
export const Slide = ({images, handdleSelectedItem, selectedItem, version2}: Props) => {
  return (
    <div className="overflow-hidden">
      <div className={" relative max-w-3xl my-0 mx-auto " + `${!version2 && "border-b border-opacity-30 border-lightBlue"}`}>
          ...
          {images.map( (img, index) => {
            return (
              <div key={index} className={"w-[100%] flex justify-center min-w-[100%] "} id={ "slide-" + index }>
                <Image className={`${version2? "h-[170px] w-full" : "h-56 w-auto"}`}   src={"/"+img} alt={"crew photograph"} width={100} height={0}/>
              </div>
            )
          }
          )}
