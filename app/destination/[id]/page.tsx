...
const Destinations = ({params}: Props) => {
  ...
  if (id || id === 0) {
    return (
      <div className="grid justify-center justify-items-center gap-12 mt-16 md:mt-28">
        <div className="navText md:text-xl md:justify-self-start">
          <p><strong className="text-lightBlue opacity-25 ">01</strong> PICK YOUR DESTINATION</p>
        </div>
        <Image src={ "/"+info.images.png} className={"h-[18.75rem] w-[18.75rem] "} width={170} height={170} alt={info.name}/>
        <DestinationNav id={id} />
        <div className="text-center mb-4">
          <h3>{info.name}</h3>
          <p className="regularText font-BarlowRegular text-center text-lightBlue max-w-[35.813rem]">
            {info.description}
          </p>
        </div>
